...
| Built-in spawn trace                                                                                                                | `<stateDir>/logs/spawn-trace.log`: time, executable, instance/purpose, exit result; arguments only as a known subcommand word, otherwise `<args>`; rotated into `.1`/`.2` at start and at ~1 MB. Patching `child_process` covers SDK launches; document any gaps                                                                                                                                                        |
