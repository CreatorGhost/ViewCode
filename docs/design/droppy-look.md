...
- The preload sets `html[data-window-glass="true"]` only while vibrancy or Mica is active (`apps/desktop/src/window/WindowGlass.ts`, `windowGlassPreload.ts`). It is cleared when the user turns off **Window transparency** (Settings → Appearance, applied live) or the OS reports Reduce transparency. Without it, the web values apply.
- ViewCode's glass sheet is denser than Droppy's for readability: black .30 (dark) / white .50 (light) plus the tint .22 / .16.
