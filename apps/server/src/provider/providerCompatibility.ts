...
export function resolveProviderCompatibility(
  ...
  const message =
    status === "broken"
      ? "This provider version is known to be incompatible with this ViewCode release."
      : status === "unsupported"
        ? "This provider version is outside the supported range for this ViewCode release."
        : status === "graceful"
          ? "This provider version has limited compatibility with this ViewCode release."
          : null;
